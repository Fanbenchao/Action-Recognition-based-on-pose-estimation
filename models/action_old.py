...
from models.activations import channel_softmax_2d
from models.blocks import build_context_aggregation
from models.layers import *
from models.objectives import elasticnet_loss_on_valid_joints
...
def _get_3d_pose_estimation_from_model(inp, model_pe, num_joints, num_blocks,
    ...
    x1 = TimeDistributed(stem, name='td_%s' % stem.name)(inp)
    xb1 = TimeDistributed(recep_block, name='td_%s' % recep_block.name)(x1)#根据论文内容需要更改
    ...
    v = add([vxy, vz])
    v = Lambda(lambda x: 2*K.expand_dims(x, axis=-1))(v) #需要实验
