...
from data_generator.datasets import get_clip_frame_index
...
def ntu_load_annotations(dataset_path, eval_mode='cs',
    ...
    ntud_numpy_dir = os.path.join(dataset_path, 'annotations/nturgb+d_numpy')
    sequences = [[], [], []]
    seq_ids = [[], [], []]
    actions = [[], [], []]
                ...
                for r in range(1,num_R+1):
                    for a in range(1,50):
                        ...
                        if eval_mode == 'cs':
                            ...
                            mode = TRAIN_MODE if p in cs_train else VALID_MODE
                        else:
                            #mode = TRAIN_MODE if c in cv_train else TEST_MODE
                            mode = TRAIN_MODE if c in cv_train else VALID_MODE
    ...
    frame_idx = [[], serialize_index_sequences(sequences[1]), serialize_index_sequences(sequences[2])]
...
class Ntu(object):
    def __init__(self, dataset_path, dataconf, poselayout=pa20j3d,
            topology='sequences', use_gt_bbox=False, remove_outer_joints=True,
            clip_size=16, pose_only=False, num_S=17):
        assert topology in ['sequences', 'frame'], \
    ...
    def get_data(self, key, mode, frame_list=None, bbox=None):
        ...
        for i in range(len(objframes)):
            if self.pose_only:
                ...
                imagepath = os.path.join(self.dataset_path, image_prefix,'test',
    ...
    def get_shape(self, dictkey):
        ...
        if dictkey == 'ntuaction':
            return (len(self.action_labels),) #由于我只是用前49个单人的动作，所以这里要减去11
